...
'INSERT INTO audit (id,ts,user,entryId,action,meta) VALUES (?,?,?,?,?,?)'
).bind(id, ts, user || 'Unknown', entryId || '', action || '', JSON.stringify(meta)).run();
}
...
await logAudit(env, b.user || 'Unknown', entryId, 'create', { date: b.date });
...
(b.caption && b.caption !== existing.caption) ||
(b.platformCaptions && JSON.stringify(b.platformCaptions) !== existing.platformCaptions) ||
(b.firstComment && b.firstComment !== existing.firstComment);
...
await logAudit(env, b.user || 'Unknown', id, 'update', { contentChanged, status });
...
await logAudit(env, 'Unknown', id, 'delete', {});
return ok({ ok: true });
};
